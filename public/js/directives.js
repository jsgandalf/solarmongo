  ...
  return {
    ...
    controller: function ( $scope, $location, Global ) {
        ...
        for(var i in $scope.navList){
            if($scope.navList[i].route == "settings" && $scope.global.user.role != 'admin' && $scope.global.user.role != 'installer' && $scope.global.user.role != 'Admin' && $scope.global.user.role != 'superadmin')
